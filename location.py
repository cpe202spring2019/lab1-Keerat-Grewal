...
# ADD BOILERPLATE HERE (__eq__ and __repr__ functions)
    def __eq__(self, other):
        return type(other) == Location and self.name == other.name and isclose(self.lat, other.lat) and \
               isclose(self.lon, other.lon)

    def __repr__(self):
        name_of_object = self.name
        latitude = self.lat
        longitude = self.lon
        return "Location('{0}', {1}, {2})".format(name_of_object, latitude, longitude)
